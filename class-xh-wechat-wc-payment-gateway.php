...
{
    private $config;
    // private $openid; // used for jsapi pay
    ...
    public function __construct()
    {
        ...
        // add the secret
        $this->config = new WechatPaymentConfig(
            $this->get_option('wechatpay_appID'),
            $this->get_option('wechatpay_mchId'),
            $this->get_option('wechatpay_key'),
            $this->get_option('wechatpay_secret')
        );
    }
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            ...
            // new: secret (for jsapipay method)
            'wechatpay_secret' => array(
                'title'       => __('WeChatPay Secret', 'wechatpay'),
                'type'        => 'text',
                'description' => __('Please enter your WeChatPay Secret; this is needed in order to take JSAPI payment.', 'wechatpay'),
                'css'         => 'width:400px',
                //'desc_tip' => true
            ),
            'exchange_rate'    => array(
    ...
    public function before_checkout_wechat_login()
    {
        if ($this->inWechat()) {
            $jsApiPay     = new JsApiPay();
            $openid = $jsApiPay->GetOpenid($this->config);

            // $_SESSION['wechat_openid'] = $openid;

            echo "<script>jQuery(document).ready(function ($) {Cookies.set('wechat_openid', '{$openid}', { expires: 7 });});</script>";

            // not work!!
            // wc_setcookie( 'wechat_openid', $openid, 1000000 );
            //setcookie( "wechat_openid", $openid, 30 * DAY_IN_SECONDS, COOKIEPATH, COOKIE_DOMAIN );
            // echo 'current openid:'. $_COOKIE["wechat_openid"];
        }
    }
    ...
    public function receipt_page($order_id)
    {
        ...
        $total = round($total * $exchange_rate, 2);
        $totalFee = (int) ($total * 100); //单位是“分”不是“元”
        ...
        // switch the native(scan qrcode in web) / jsapi(wechat embed browser)
        if ($this->inWechat()) {
            $input->SetTrade_type("JSAPI");
            
            // $openid = $_SESSION["wechat_openid"];
            $openid = $_COOKIE["wechat_openid"];
        ...
        if ($this->inWechat()) {
	        $jsApiPay        = new JsApiPay();
	        $jsApiParameters = $jsApiPay->GetJsApiParameters($result, $this->config);
	        
	        $editAddress = $jsApiPay->GetEditAddressParameters($this->config);

	    	echo '<p>待支付' . $total .' 元</p>';
	    	echo '<button type="button" class="button button-primary" onclick="callpay()">立即支付</button>';
	    	
	    	// pass the data to jsapi script(jsapi.pay.js)
	    	echo "<script>var jsApiParameters = ". $jsApiParameters .";var editAddress = " . $editAddress .  "</script>";
	    	echo "<script src='" . XH_WC_WECHAT_URL . '/js/jsapi.pay.min.js' .  "'></script>";

	    	// hide the qrcode when in wechat
	    	echo '<div style="display:none;" id="xh-wechat-payment-pay-img" data-oid="' . $order_id . '"></div>';
	    } else {
